...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: getHreflangMetadata(`/blog/${params.slug}`, locale),
    openGraph: {
      title: b.seoTitle || b.title,
      description: b.seoDescription || b.excerpt || b.title,
...
function toHtml(content: string) {
  ...
  text = text.replace(
    /!\[([^\]]*)\]\((https?:\/\/[^\s\)\<\>]+|\/[^\s\)\<\>]+)\)/g,
    '<img src="$2" alt="$1" class="rounded-xl my-4 max-w-full h-auto" />'
  );
  ...
  text = text.replace(
    ...
    (match, label, url) => {
      ...
      } class="text-brand-600 hover:text-brand-700 underline font-medium">${label}</a>`;
    }
  );
...
export default async function BlogDetail({
  ...
  const localizedArticleUrl = `${base}${getLocalizedPath(`/blog/${b.slug}`, locale)}`;
  ...
  const schema = {
    ...
    description: b.seoDescription || b.excerpt || b.title,
    keywords: tags.length > 0 ? tags.join(", ") : undefined,
    mainEntityOfPage: { "@type": "WebPage", "@id": localizedArticleUrl }
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: t.home, item: `${base}${getLocalizedPath("/", locale)}` },
      {
        ...
        position: 2,
        name: t.blog,
        item: `${base}${getLocalizedPath("/blog", locale)}`
      },
      { "@type": "ListItem", position: 3, name: b.title, item: localizedArticleUrl }
    ]
  };
    ...
    <div className="bg-white">
      ...
      <div className="container-x pt-6">
        <nav className="text-sm text-ink-500 mb-6 flex flex-wrap items-center gap-1.5">
          ...
          <span>/</span>
          <Link href={getLocalizedPath("/blog", locale)} className="hover:text-brand-600 transition">
            {t.blog}
          </Link>
          <span>/</span>
      ...
      <article className="container-x max-w-3xl pb-12">
        {b.category?.name && (
          <Link
            href={`${getLocalizedPath("/blog", locale)}?category=${b.category.slug}`}
            className="inline-block text-xs font-bold uppercase tracking-wide text-brand-600 mb-2"
          >
            {b.category.name}
          </Link>
        )}
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-ink-500 mt-4 pb-6 border-b border-ink-100">
          ...
          <span className="flex items-center gap-1">
            <Clock size={14} /> {readingTime(html)} {t.minRead}
          </span>
        </div>
        ...
        {tags.length > 0 && (
          <div className="mt-10 flex flex-wrap gap-2">
            {tags.map(tag => (
              <Link
                key={tag}
                href={`${getLocalizedPath("/blog", locale)}?q=${encodeURIComponent(tag)}`}
                className="text-xs bg-ink-100 text-ink-700 px-3 py-1 rounded-full hover:bg-brand-50 hover:text-brand-700"
              >
                #{tag}
              </Link>
            ))}
          ...
          <div className="container-x py-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {related.map(r => (
                <Link
                  key={r.id}
                  href={getLocalizedPath(`/blog/${r.slug}`, locale)}
                  className="group bg-white rounded-lg border border-ink-100 overflow-hidden hover:shadow-hover transition"
                >
                  <div className="aspect-[16/9] overflow-hidden">
                    <img
                      src={r.coverImage || `https://picsum.photos/seed/${r.slug}/600/340`}
                      alt={r.title}
