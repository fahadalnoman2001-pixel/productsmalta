...
// dispatch(prisma, name, args) runs the tool against the shared database.
...
export const tools = [
  {
    ...
    inputSchema: { type: "object", properties: {
      ...
      brand: { type: "string" }, platform: { type: "string" },
      isFeatured: { type: "boolean" }, isBestSeller: { type: "boolean" },
      search: { type: "string" }, limit: { type: "integer", default: 100 }
    } }
  },
  {
    name: "get_product",
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["title","description","price","affiliateUrl"], properties: {
      title: { type: "string" }, slug: { type: "string" },
      description: { type: "string" }, shortDesc: { type: "string" },
      images: { type: "array", items: { type: "string" } },
      price: { type: "number" }, originalPrice: { type: "number" },
      currency: { type: "string", default: "EUR" },
      brand: { type: "string" }, platform: { type: "string" },
      affiliateUrl: { type: "string" },
      rating: { type: "number" }, reviewCount: { type: "integer" },
      categorySlug: { type: "string" }, subcategorySlug: { type: "string" },
  ...
  },
  {
    ...
    inputSchema: { type: "object", properties: { id: { type: "string" }, slug: { type: "string" }, patch: { type: "object" } }, required: ["patch"] }
  },
  {
    name: "delete_product",
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["title","content"], properties: {
      ...
      coverImage: { type: "string" }, author: { type: "string" },
      categorySlug: { type: "string" }, tags: { type: "array", items: { type: "string" } },
      seoTitle: { type: "string" }, seoDescription: { type: "string" },
      isPublished: { type: "boolean", default: true }, isTop: { type: "boolean" }
    } }
  },
  {
    name: "update_blog",
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["name"], properties: { name: { type: "string" }, slug: { type: "string" }, description: { type: "string" }, image: { type: "string" }, showOnHomepage: { type: "boolean", description: "show this category's product row on the homepage" } } }
  },
  {
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["name","categorySlug"], properties: { name: { type: "string" }, slug: { type: "string" }, categorySlug: { type: "string" } } }
  },
  {
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["name"], properties: { name: { type: "string" }, slug: { type: "string" }, type: { type: "string" }, description: { type: "string" }, showOnHomepage: { type: "boolean", description: "show this collection as a row on the homepage" } } }
  },
  {
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["collectionSlug","productSlug"], properties: { collectionSlug: { type: "string" }, productSlug: { type: "string" } } }
  },
  {
  ...
  },
  {
    ...
    inputSchema: { type: "object", required: ["title","image"], properties: {
      title: { type: "string" }, subtitle: { type: "string" }, image: { type: "string" }, link: { type: "string" },
      slot: { type: "string", default: "hero" }, slotKey: { type: "string" }, order: { type: "integer" }
    } }
  },
  {
    name: "delete_banner",
...
export async function dispatch(prisma, name, args) {
  switch (name) {
    case "list_products": {
      ...
      if (args.category) { const c = await prisma.category.findUnique({ where: { slug: args.category } }); if (c) where.categoryId = c.id; }
      return prisma.product.findMany({ where, include: { category: true, subcategory: true }, take: args.limit || 100, orderBy: { createdAt: "desc" } });
    }
    case "get_product": return findProduct(prisma, args);
    case "create_product": {
      ...
      return prisma.product.create({ data: {
        title: args.title, slug: args.slug || slugify(args.title),
        description: args.description, shortDesc: args.shortDesc,
        images: JSON.stringify(args.images || []),
        price: args.price, originalPrice: args.originalPrice,
        currency: args.currency || "EUR",
        brand: args.brand, platform: args.platform, affiliateUrl: args.affiliateUrl,
        rating: args.rating ?? 4.5, reviewCount: args.reviewCount ?? 0,
        categoryId: cat?.id, subcategoryId: sub?.id,
        tags: JSON.stringify(args.tags || []),
        seoTitle: args.seoTitle, seoDescription: args.seoDescription,
        isFeatured: !!args.isFeatured, isBestSeller: !!args.isBestSeller
      }});
    }
    case "update_product": {
      const p = await findProduct(prisma, args); if (!p) throw new Error("product not found");
      ...
      if (patch.categorySlug) { const c = await prisma.category.findUnique({ where: { slug: patch.categorySlug } }); if (c) patch.categoryId = c.id; delete patch.categorySlug; }
      return prisma.product.update({ where: { id: p.id }, data: patch });
    }
    case "delete_product": {
      const p = await findProduct(prisma, args); if (!p) throw new Error("product not found");
      await prisma.product.delete({ where: { id: p.id } }); return { deleted: true };
    }
    case "search_products":
      return prisma.product.findMany({
        where: { isActive: true, OR: [{ title: { contains: args.query } }, { description: { contains: args.query } }] },
        include: { category: true }, take: args.limit || 20
      });
    ...
    case "create_blog": {
      ...
      return prisma.blog.create({ data: {
        title: args.title, slug: args.slug || slugify(args.title),
        excerpt: args.excerpt, content: args.content, coverImage: args.coverImage,
        author: args.author || "Admin", categoryId: cat?.id,
        tags: JSON.stringify(args.tags || []),
        seoTitle: args.seoTitle, seoDescription: args.seoDescription,
        isPublished: args.isPublished !== false, isTop: !!args.isTop
      }});
    }
    case "update_blog": {
      const b = await findBlog(prisma, args); if (!b) throw new Error("blog not found");
      const patch = { ...args.patch };
      if (patch.tags && Array.isArray(patch.tags)) patch.tags = JSON.stringify(patch.tags);
      if (patch.categorySlug) { const c = await prisma.category.findUnique({ where: { slug: patch.categorySlug } }); if (c) patch.categoryId = c.id; delete patch.categorySlug; }
    ...
    case "delete_blog": {
      const b = await findBlog(prisma, args); if (!b) throw new Error("blog not found");
      await prisma.blog.delete({ where: { id: b.id } }); return { deleted: true };
    }

    case "list_categories":
      return prisma.category.findMany({ include: { subcategories: true }, orderBy: { order: "asc" } });
    case "create_category":
      return prisma.category.create({ data: { name: args.name, slug: args.slug || slugify(args.name), description: args.description, image: args.image, showOnHomepage: args.showOnHomepage !== false } });
    case "update_category": {
      const c = await prisma.category.findUnique({ where: { slug: args.slug } });
      if (!c) throw new Error("category not found");
      const patch = { ...args.patch }; delete patch.subcategories;
    ...
    case "delete_category":
      await prisma.category.delete({ where: { slug: args.slug } }); return { deleted: true };

    case "list_collections":
      return prisma.collection.findMany({ include: { products: { include: { product: true } } }, orderBy: { order: "asc" } });
    case "create_collection":
      return prisma.collection.create({ data: { name: args.name, slug: args.slug || slugify(args.name), type: args.type || "manual", description: args.description, showOnHomepage: args.showOnHomepage !== false } });
    case "update_collection": {
      const c = await prisma.collection.findUnique({ where: { slug: args.slug } });
      if (!c) throw new Error("collection not found");
      const patch = { ...args.patch }; delete patch.products;
    ...
    case "add_product_to_collection": {
      ...
      return prisma.collectionProduct.upsert({
        ...
        update: {}, create: { collectionId: c.id, productId: p.id }
      });
    }
    ...
    case "create_banner":
      return prisma.banner.create({ data: {
        title: args.title, subtitle: args.subtitle, image: args.image, link: args.link,
        slot: args.slot || "hero", slotKey: args.slotKey, order: args.order || 0
      }});
    case "delete_banner":
      await prisma.banner.delete({ where: { id: args.id } }); return { deleted: true };
    ...
    case "get_dashboard_stats": {
      const [products, blogs, cats, cols, clicks] = await Promise.all([
        prisma.product.count(), prisma.blog.count(), prisma.category.count(),
        prisma.collection.count(), prisma.clickLog.count()
      ]);
      return { products, blogs, categories: cats, collections: cols, clicks };
    }
